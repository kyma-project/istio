...
## Scenario 1: Configure Rate Limiting Using Dynamic Header Values
...
1. Apply the configuration
    ...
    ```
2. Restart the rate limit service to apply the new configuration:
    ```bash
    kubectl delete pod -n ratelimit -l app=ratelimit
    ```
4. Test rate limiting. Each `user-id` has its own request limit.
    ```bash
    ...
    You get the following error indicating that the key is too long:
    ```
...
## Scenario 2: Configure Rate Limiting Using the Client Certificate
There is no out-of-the-box [Envoy RateLimit Action](https://www.envoyproxy.io/docs/envoy/latest/api-v3/config/route/v3/route_components.proto#config-route-v3-ratelimit-action) that supports the **X-Forwarded-Client-Cert** header. In this example, **request_headers**  is used to extract the value from the **X-Forwarded-Client-Cert** header.
...
2. Restart the rate limit service to apply the new configuration.
    ...
    kubectl delete pod -n ratelimit -l app=ratelimit
    ```
3. To test rate limiting, use **X-Forwarded-Client-Cert** added by Ingress Gateway.
    ```bash
    curl -i -X GET "http://httpbin.ps-rate.goatz.shoot.canary.k8s-hana.ondemand.com/get"
    ```

    Since rate limiting is applied on the sidecar, the client certificate is not working due to too long key value:
    ```
