...
- **Intermediate CAs**: A certificate issued by the Root CA to each Kubernetes cluster running Istio. It is recommended to keep the Intermediate CA certificates as short-lived as possible.
- **Workload Certificates**: Automatically generated by Istio for each Pod and signed by the cluster's intermediate CA.

To configure a custom certificate in Kyma, in each cluster's `istio-system` namespace, you must create a Secret `cacerts`. In order to create the Secret, you need the following input:
