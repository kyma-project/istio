Feature: X-Forwarded-For header
  ...
  Scenario: X-Forward-For header does not contain public client IP when externalTrafficPolicy is not set in the Istio CR
    ...
    Then Request sent to exposed httpbin, "should not" contain public client IP in "X-Forwarded-For" header
  ...
  Scenario: X-Forward-For header does not contain public client IP when externalTrafficPolicy is set to Cluster
    Given Istio CR "istio-sample" is applied in namespace "kyma-system" with externalTrafficPolicy set to "Cluster"
    And Istio CR "istio-sample" in namespace "kyma-system" has status "Ready"
    And Istio injection is "enabled" in namespace "default"
    And Httpbin application "httpbin" deployment is created in namespace "default"
    And "Deployment" "httpbin" in namespace "default" is ready
