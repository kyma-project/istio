...
func initScenario(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^Istio CR "([^"]*)" is applied in namespace "([^"]*)" with externalTrafficPolicy set to "([^"]*)"$`, steps.CreateIstioWithExternalTrafficPolicy)
	...
	ctx.Step(`^Request sent to exposed httpbin, "([^"]*)" contain public client IP in "([^"]*)" header$`, steps.ValidatePublicClientIpInHeader)
