...
test: manifests generate fmt vet envtest ## Run tests.
	KUBEBUILDER_CONTROLPLANE_START_TIMEOUT=2m KUBEBUILDER_CONTROLPLANE_STOP_TIMEOUT=2m KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) -p path)" go test $(shell go list ./... | grep -v /api/v1alpha1/groupversion_info.go | grep -v /tests/integration | grep -v /api/v1alpha1/zz_generated.deepcopy.go) -coverprofile cover.out
