...
istio_module_template_count=$(kubectl get moduletemplates.operator.kyma-project.io -A --output json | jq -j '.items | length')
...
kyma_cr_name=$(kubectl get kyma -n kyma-system -l operator.kyma-project.io/managed-by=lifecycle-manager --no-headers -o custom-columns=":metadata.name")
...
if [ "$istio_crs" -gt 0 ]; then
  ...
  #TODO: As far as I remember we should not remove the customResourcePolicy after migration, is this still correct?
else
  echo "Istio CR not found, proceeding with migration by adding Istio module to Kyma CR"
