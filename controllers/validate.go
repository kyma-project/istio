...
func (r *IstioReconciler) validate(istioCR *operatorv1alpha2.Istio) described_errors.DescribedError {
	if istioCR.Spec.Experimental != nil {
		...
		return described_errors.NewDescribedError(errors.New("istio CR contains experimental feature"), "Experimental features are not supported in this image flavour").
			SetWarning().
			SetCondition(false)
	}
	return nil
