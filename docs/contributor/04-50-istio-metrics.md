...
| **istio_forward_client_cert_details_setting**          | Indicates whether **forwardClientCertDetails** is configured and which value is set in the Istio CR. (`1` when configured, `0` otherwise). The `value` label which refer to value of setting, value can be one of: `ALWAYS_FORWARD_ONLY`, `APPEND_FORWARD`, `FORWARD_ONLY`, `SANITIZE`, `SANITIZE_SET`. |
