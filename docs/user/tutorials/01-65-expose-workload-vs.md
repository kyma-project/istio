...
- You have set up your custom domain and a custom TLS Gateway. See [Configure a TLS Gateway in SAP BTP, Kyma Runtime](https://kyma-project.io/external-content/api-gateway/docs/user/istio-gateways/set-up-tls-gateway.html). <br>Alternatively, you can use the default domain of your Kyma cluster and the default Gateway `kyma-system/kyma-gateway`. To use the default domain and Gateway, the API Gateway module must be added to your cluster.
