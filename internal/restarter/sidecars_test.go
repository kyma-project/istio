...
var _ = Describe("SidecarsRestarter reconciliation", func() {
	...
	It("should succeed proxy reset even if more than 5 proxies could not be reset and will return a warning", func() {
		...
		Expect(
			(*istioCr.Status.Conditions)[0].Message,
		).To(ContainSubstring("The sidecars of the following workloads could not be restarted: ns1/name1, ns2/name2, ns3/name3, ns4/name4, ns5/name5 and 1 additional workload(s)"))
		Expect((*istioCr.Status.Conditions)[0].Status).To(Equal(metav1.ConditionFalse))
	})
...
func (p *proxyRestarterMock) RestartProxies(
	_ context.Context,
	_ predicates.SidecarImage,
...
func (p *proxyRestarterMock) RestartWithPredicates(
	_ context.Context,
	preds []predicates.SidecarProxyPredicate,
	_ *pods.RestartLimits,
	_ bool,
) ([]restart.Warning, bool, error) {
	return p.restartWarnings, p.hasMorePods, p.err
}
