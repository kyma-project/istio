...
type Config struct {
	...
	// GatewayExternalTrafficPolicy defines the external traffic policy for Istio Ingress Gateway Service. Valid configurations are "Local" or "Cluster". An external traffic policy set to "Local  preserves the client IP in the request, but also introduces the risk of unbalanced traffic distribution.
	// +kubebuilder:validation:Optional
