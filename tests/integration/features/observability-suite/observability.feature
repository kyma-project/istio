Feature: Observability
  ...
  Scenario: Logs from stdout-json envoyFileAccessLog provider are in correct format
    ...
    And Request with header "Host" with value "httpbin.default.svc.cluster.local" to path "/ip" should have response code "200"
  ...
  Scenario: Istio calls OpenTelemetry API on default service configured in kyma-traces extension provider
    ...
    When Request with header "Host" with value "httpbin.default.svc.cluster.local" to path "/headers" should have response code "200"
